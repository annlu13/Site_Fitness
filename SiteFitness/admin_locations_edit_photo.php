<?php
	...
	if(isset($_GET['a'])){
		if($_GET['a']=='del' && isset($_GET['imgid'])){
			$q = "DELETE FROM locations_pic WHERE ID={$_GET["imgid"]};";
			$db->exec($q);
			header("Location: admin_locations_edit_photo.php?id={$_GET['id']}");
		}
		if($_GET['a']=='add'){
			$img = $_POST["img"];
			$q = "INSERT INTO locations_pic (date, img, locationID) VALUES (NOW(), '{$img}', {$_GET['id']})";
			$db->exec($q);
			header("Location: admin_locations_edit_photo.php?id={$_GET['id']}");
		}
	}
...
<body>
	...
	<main class="part1">
		<h1>Залы</h1>
		<p><span>Название зала: </span><?php echo($res[0]['name']); ?></p>
		<form method="POST" action="admin_locations_edit_photo.php?a=add&id=<?php echo($_GET['id']); ?>">
